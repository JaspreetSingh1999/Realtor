var Campground = require("../models/property");
...
middlewareObj.checkShareOwnership = function(req, res, next) {
  Share.findById(req.params.comment_id, function(err, foundComment) {
    if (err || !foundComment) {
      req.flash("error", "Sorry, that share does not exist!");
      res.redirect("/home");
    } else if (
      foundComment.author.id.equals(req.user._id) ||
      req.user.isAdmin
...
middlewareObj.checkProfileOwnership = function(req, res, next) {
  User.findById(req.params.user_id, function(err, foundUser) {
    if (err || !foundUser) {
      ...
      req.flash("error", "You don't have permission to do that!");
      res.redirect("/properties/purpose/" + req.params.user_id);
    }
  });
};
